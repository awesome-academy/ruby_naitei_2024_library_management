module Components::TableHelper
  def render_table(caption = nil, **options, &block)
    content_tag :table, options.merge(
      ...
      if caption.present?
        content_tag :caption, caption,
                    class: "mt-4 text-sm text-muted-foreground " do
          capture(&block)
        end
      else
        capture(&block)
    ...
    content_tag :thead, options.merge(
      ...
      content_tag :tr,
                  class: "border-b transition-colors hover:bg-muted/50
                    data-[state=selected]:bg-muted" do
        capture(&block)
      end
    end
  end
  ...
  def table_header(content = nil, **options, &block)
    content_tag :th, options.merge(
      class: tw("h-12 px-4 text-left align-middle font-medium
        text-muted-foreground [&:has([role=checkbox])]:pr-0", options[:class])
    ) do
      if block
        capture(&block)
      else
  ...
  def table_row(**options, &block)
    content_tag :tr, options.merge(
      class: tw("border-b transition-colors hover:bg-muted/50
        data-[state=selected]:bg-muted", options[:class])
    ), &block
  end

  def table_column(content = nil, **options, &block)
    content_tag :td, options.merge(
      class: tw("p-4 align-middle [&:has([role=checkbox])]:pr-0 font-medium",
                options[:class])
    ) do
      if block
        capture(&block)
